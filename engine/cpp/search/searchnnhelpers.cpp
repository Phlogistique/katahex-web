...
void Search::computeRootNNEvaluation(NNResultBuf& nnResultBuf) {
  ...
  nnInputParams.policyLocalFocusDist = searchParams.policyLocalFocusDist;
  nnInputParams.policyLocalFocusPow = searchParams.policyLocalFocusPow;
...
//Async counterpart of initNodeNNOutput for a leaf this thread has just claimed by setting
//STATE_EVALUATING: queue the eval and return PENDING, or on a cache hit fill the node in
//and return FINISHED. The caller expands FINISHED nodes; PENDING ones expand on completion.
PlayoutResult Search::beginLeafNNEvaluation(SearchThread& thread, SearchNode& node) {
  MiscNNInputParams nnInputParams;
  nnInputParams.noResultUtilityForWhite = searchParams.noResultUtilityForWhite;
  nnInputParams.nnPolicyTemperature = searchParams.nnPolicyTemperature;
  nnInputParams.policyLocalFocusDist = searchParams.policyLocalFocusDist;
  ...
  PendingPlayoutEval* pending = thread.freeSlots.back();
  thread.freeSlots.pop_back();
  pending->board = thread.board;
  pending->history = thread.history;
  pending->pla = thread.pla;
  pending->nnInputParams = nnInputParams;
  pending->leaf = &node;
  pending->path.clear();

  bool inFlight = nnEvaluator->beginEvaluate(
    pending->board, pending->history, pending->pla,
    pending->nnInputParams, pending->buf, false
  );
  if(!inFlight) {
    //Cache hit, the result is already complete.
    std::shared_ptr<NNOutput>* result = new std::shared_ptr<NNOutput>(std::move(pending->buf.result));
    node.nodeAge.store(searchNodeAge,std::memory_order_release);
    bool suc = node.storeNNOutputIfNull(result);
    if(!suc)
      delete result;
      ...
      addCurrentNNOutputAsLeafValue(node,true);
    thread.freeSlots.push_back(pending);
    return PlayoutResult::FINISHED;
  }
  thread.currentPending = pending;
  return PlayoutResult::PENDING;
}

//Complete the oldest queued leaf eval: expand the leaf, then replay the deferred unwind of
//edge visits, stats updates, and virtual losses that a synchronous playout does as its
//recursion returns. Returns whether this counted as a finished playout.
bool Search::finishOldestPendingEval(SearchThread& thread) {
  assert(!thread.pendingEvals.empty());
  PendingPlayoutEval* pending = thread.pendingEvals.front();
  thread.pendingEvals.pop_front();

  nnEvaluator->finishEvaluate(
    pending->board, pending->history, pending->pla,
    pending->nnInputParams, pending->buf
  );

  SearchNode& node = *pending->leaf;
  std::shared_ptr<NNOutput>* result = new std::shared_ptr<NNOutput>(std::move(pending->buf.result));
  node.nodeAge.store(searchNodeAge,std::memory_order_release);
  bool suc = node.storeNNOutputIfNull(result);
  if(!suc)
  ...
  for(const PendingPlayoutEval::PathLink& link : pending->path) {
    int nodeState = link.node->state.load(std::memory_order_acquire);
    int childrenCapacity;
    SearchChildPointer* children = link.node->getChildren(nodeState,childrenCapacity);
    assert(childrenCapacity > link.childIdx);
    children[link.childIdx].addEdgeVisits(1);
    updateStatsAfterPlayout(*link.node,thread,link.node == rootNode);
    link.child->virtualLosses.fetch_add(-1,std::memory_order_release);
  }
