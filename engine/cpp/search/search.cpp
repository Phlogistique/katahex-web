...
PendingPlayoutEval::PendingPlayoutEval()
  :buf(),board(),history(),pla(C_EMPTY),nnInputParams(),leaf(NULL),path()
{}
   ...
   currentPending(NULL)
{
  ...
  if(search.searchParams.numLeafEvalsPerThread >= 2) {
    for(int i = 0; i < search.searchParams.numLeafEvalsPerThread; i++) {
      pendingPool.push_back(std::make_unique<PendingPlayoutEval>());
      freeSlots.push_back(pendingPool.back().get());
    }
  }
  ...
  std::function<void(int)> searchLoop = [
    ...
    try {
      ...
      while(true) {
        ...
        PlayoutResult result = runSinglePlayout(*stbuf, upperBoundVisitsLeft);
        bool finishedPlayout;
        if(result == PlayoutResult::FINISHED)
          finishedPlayout = true;
        else if(result == PlayoutResult::PENDING)
          //Keep starting playouts until the allowed number of evals is in flight, then finish
          //the oldest, which has had the longest to complete.
          finishedPlayout = stbuf->freeSlots.empty() && finishOldestPendingEval(*stbuf);
        else if(!stbuf->pendingEvals.empty())
          //A failed playout often ran into one of our own pending leaves; completing it unsticks us.
          finishedPlayout = finishOldestPendingEval(*stbuf);
        else {
      ...
      //Finish any leaf evals still in flight, completing their playouts.
      while(!stbuf->pendingEvals.empty()) {
        if(finishOldestPendingEval(*stbuf))
          numPlayoutsShared.fetch_add((int64_t)1, std::memory_order_relaxed);
      }
    }
    catch(...) {
      //The server threads hold pointers into the pending bufs, so those evals must resolve
      //before the thread state dies.
      try {
        while(!stbuf->pendingEvals.empty())
          finishOldestPendingEval(*stbuf);
      }
      catch(...) {}
...
PlayoutResult Search::runSinglePlayout(SearchThread& thread, double upperBoundVisitsLeft) {
  ...
  PlayoutResult result = playoutDescend(thread,*rootNode,posesWithChildBuf,true);
  if(result == PlayoutResult::PENDING) {
    thread.pendingEvals.push_back(thread.currentPending);
    thread.currentPending = NULL;
  }
...
PlayoutResult Search::playoutDescend(
  ...
  if(nodeState == SearchNode::STATE_UNEVALUATED) {
    if(!isRoot && !thread.freeSlots.empty()) {
      //Claim the node so nobody else evaluates it, then queue the eval and go start another
      //playout instead of blocking; the node expands when the result is collected.
      bool suc = node.state.compare_exchange_strong(nodeState, SearchNode::STATE_EVALUATING, std::memory_order_seq_cst);
      if(!suc)
        return PlayoutResult::FAILED;
      PlayoutResult result = beginLeafNNEvaluation(thread,node);
      if(result == PlayoutResult::FINISHED) {
  ...
  //Recurse!
  PlayoutResult result = playoutDescend(thread,*child,posesWithChildBuf,false);
  ...
  if(result == PlayoutResult::PENDING) {
    //The pending eval will replay this update when it completes, so hold the virtual loss.
    thread.currentPending->path.push_back(PendingPlayoutEval::PathLink{&node,bestChildIdx,child});
    return result;
  }
  child->virtualLosses.fetch_add(-1,std::memory_order_release);
