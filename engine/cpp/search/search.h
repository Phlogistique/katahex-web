...
//What a single playout did. PENDING means it reached a new leaf and submitted its neural
//net evaluation, and the rest of the playout waits in thread.pendingEvals: virtual losses
//stay applied along the path, and edge visits and stats update when the eval completes.
enum class PlayoutResult { FAILED, FINISHED, PENDING };

//A leaf evaluation in flight on the GPU, with everything needed to finish the playout.
struct PendingPlayoutEval {
  NNResultBuf buf;
  Board board; //Board and history at the leaf, for postprocessing the result
  BoardHistory history;
  Player pla;
  MiscNNInputParams nnInputParams;
  SearchNode* leaf;
  struct PathLink { SearchNode* node; int childIdx; SearchNode* child; };
  std::vector<PathLink> path; //Leaf-to-root order, as the recursive unwind would visit them

  PendingPlayoutEval();
  PendingPlayoutEval(const PendingPlayoutEval&) = delete;
  PendingPlayoutEval& operator=(const PendingPlayoutEval&) = delete;
};
...
struct SearchThread {
  ...
  //Slots for leaf evals in flight, sized by numLeafEvalsPerThread when it is >= 2.
  std::vector<std::unique_ptr<PendingPlayoutEval>> pendingPool;
  std::vector<PendingPlayoutEval*> freeSlots;
  std::deque<PendingPlayoutEval*> pendingEvals; //In flight, oldest first
  PendingPlayoutEval* currentPending; //Set by playoutDescend when it returns PENDING
...
struct Search {
  ...
  bool finishOldestPendingEval(SearchThread& thread); //Returns whether it counted as a finished playout
...
private:
  ...
  PlayoutResult beginLeafNNEvaluation(SearchThread& thread, SearchNode& node);
