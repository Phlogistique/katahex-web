...
void NNEvaluator::setServeBatchWaitMicros(int micros) {
  serveBatchWaitMicros = micros;
}
...
void NNEvaluator::evaluate(
  ...
  if(beginEvaluate(board,history,nextPlayer,nnInputParams,buf,skipCache))
    finishEvaluate(board,history,nextPlayer,nnInputParams,buf);
}

bool NNEvaluator::beginEvaluate(
  Board& board,
  const BoardHistory& history,
  Player nextPlayer,
  const MiscNNInputParams& nnInputParams,
...
void NNEvaluator::finishEvaluate(
  Board& board,
  const BoardHistory& history,
  Player nextPlayer,
  const MiscNNInputParams& nnInputParams,
  ...
  Hash128 nnHash = NNInputs::getHash(board, history, nextPlayer, nnInputParams);
  //Default-constructed is what init produces on Hex, where it computes nothing.
  GameLogic::ResultsBeforeNN resultsBeforeNN;
  resultsBeforeNN.init(board, history, nextPlayer);
