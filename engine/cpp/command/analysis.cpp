...
#ifdef KATAHEX_QUERY_QUEUE
#include <emscripten.h>
#include <condition_variable>
#include <deque>
#include <mutex>

// A page has no stdin. Reading it would be proxied to the thread running the
// page's JavaScript, and blocking that thread stops every other thread's output
// from getting out, so the browser build takes its queries by a call instead.
namespace {
  mutex queryMutex;
  condition_variable queryArrived;
  deque<string> queries;
}

extern "C" EMSCRIPTEN_KEEPALIVE void katahexPushQuery(const char* query) {
  {
    lock_guard<mutex> lock(queryMutex);
    queries.emplace_back(query);
  }
  queryArrived.notify_one();
}

// Never reports the end of input: the page stops the engine by dropping it.
static bool getNextQuery(string& line) {
  unique_lock<mutex> lock(queryMutex);
  queryArrived.wait(lock, []() { return !queries.empty(); });
  line = std::move(queries.front());
  queries.pop_front();
  return true;
}
#else
...
int MainCmds::analysis(const vector<string>& args) {
  ...
  NNEvaluator* nnEval;
  {
    ...
    const int maxConcurrentEvals = numAnalysisThreads * defaultParams.numThreads * defaultParams.numLeafEvalsPerThread * 2 + 16; // * 2 + 16 just to give plenty of headroom
