...
void SearchParams::failIfParamsDifferOnUnchangeableParameter(const SearchParams& initial, const SearchParams& dynamic) {
  ...
  if(dynamic.numLeafEvalsPerThread > initial.numLeafEvalsPerThread) {
    throw StringError("Cannot increase numLeafEvalsPerThread after initialization since this is used to initialize neural net buffer capacity");
  }
...
void SearchParams::printParams(std::ostream& out) {
  ...
  PRINTPARAM(numLeafEvalsPerThread);
