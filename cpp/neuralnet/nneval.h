...
class NNEvaluator {
 public:
  ...
  //The two halves of evaluate, for callers that want several evaluations in flight at once.
  //beginEvaluate queues the position and returns true, or returns false on a cache hit with
  //the result already complete in buf. If it returned true, finishEvaluate must eventually be
  //called with the same arguments (buf must stay alive and unmoved in between).
  bool beginEvaluate(
    Board& board,
    const BoardHistory& history,
    Player nextPlayer,
  ...
  //How long a server thread lets a partial batch keep filling before running it.
  //Set before spawnServerThreads. Zero runs whatever is there, as always.
  void setServeBatchWaitMicros(int micros);
