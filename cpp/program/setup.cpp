...
vector<NNEvaluator*> Setup::initializeNNEvaluators(
  ...
  for(size_t i = 0; i<nnModelFiles.size(); i++) {
    ...
    if(cfg.contains("nnServeBatchWaitMicros"))
      nnEval->setServeBatchWaitMicros(cfg.getInt("nnServeBatchWaitMicros", 0, 1000000));
...
vector<SearchParams> Setup::loadParams(
  ...
  for(int i = 0; i<numBots; i++) {
    ...
    if(cfg.contains("numLeafEvalsPerThread"+idxStr)) params.numLeafEvalsPerThread = cfg.getInt("numLeafEvalsPerThread"+idxStr, 1, 256);
    else if(cfg.contains("numLeafEvalsPerThread"))   params.numLeafEvalsPerThread = cfg.getInt("numLeafEvalsPerThread",        1, 256);
    else                                             params.numLeafEvalsPerThread = 1;
